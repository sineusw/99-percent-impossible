/* 99% IMPOSSIBLE — HTML5 Audio SFX Engine v4.0.1 (Strict 3-Asset Minimal)
   Exactly 3 game outcomes: fail, win, perfect. No start/tick/go/tap hooks.
   Global mute now silences gameplay outcome SFX too. */
(()=>{
'use strict';
...
function playFresh(name){if(!soundOn())return false;const src=SFX_MAP[name];if(!src)return false;try{const sound=new Audio(src);sound.preload='auto';sound.playsInline=true;sound.setAttribute('playsinline','');sound.volume=name==='perfect'?.75:.6;const p=sound.play();p?.catch?.(()=>{});return true}catch{return false}}
