...
module.exports = async function handler(req, res) {
  ...
  if (typeof sessionId !== 'string' || !sessionId.startsWith('cs_') ||
      typeof buyerKey !== 'string' || buyerKey.length < 20 || buyerKey.length > 200) {
  ...
  try {
    ...
    if (!stripeRes.ok) return json(res, 400, { error: 'invalid_session' });

    const cast = session?.metadata?.cast;
    ...
    if (!paid || !buyerMatches || !ALLOWED_CAST.has(cast)) {
      return json(res, 403, { error: 'purchase_not_verified' });
    }

    const payload = {
      v: 1,
      cast,
      buyerKeyHash: expectedBuyerHash,
      purchase: session.id,
