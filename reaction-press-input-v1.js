/* 99% IMPOSSIBLE — Reaction physical-press fairness v1.2.1
   Uses touchstart/pointerdown and passes a normalized performance-clock timestamp into rxHit(). */
(()=>{
'use strict';
const play=document.querySelector('#play');if(!play||typeof window.rxHit!=='function')return;
...
function scorePress(e){if(!canScore()||pressScored)return false;e.preventDefault?.();safeResumeAudio();pressScored=true;window.rxHit(pressTime(e));setTimeout(()=>{pressScored=false},170);return true}
...
play.onpointerup=e=>{if(touchHandled||pressScored){e.preventDefault?.();return}if(canScore()){e.preventDefault?.();safeResumeAudio();window.rxHit(performance.now())}};
