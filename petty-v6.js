/* 99% IMPOSSIBLE — Petty v6.5: simple funny greetings + gesture-safe Older Joe welcome */
...
// Preload immediately. The welcome itself is allowed to fire only from a real
// pointer gesture using the already-buffered Older Joe blob. If the blob is not
// ready on the first tap, wait for the next tap instead of falling back to robot TTS.
...
addEventListener('pointerdown',e=>{
  ...
  if(typeof window.playPreloadedPettyVoice==='function'){
    if(window.playPreloadedPettyVoice(g)){said=true;return}
    return;
  }
  // Legacy fallback only when Older Joe transport is unavailable entirely.
  said=true;
