...
const PRE_AD_PAUSE=4700,SIM_AD_SECONDS=5,POST_AD_PAUSE=650;
const BEFORE_LINES=['And now, a brief message from the people funding your suffering.','Do not go anywhere. Capitalism has requested a moment.','Please enjoy this advertisement while I review your performance.','We will return shortly to your regularly scheduled disappointment.','A commercial break. Apparently humiliation alone does not pay the bills.','Hold that thought. Someone has paid to interrupt your suffering.'];
const AFTER_LINES=['Welcome back to your suffering.','And we return to the experiment.','Commercial break complete. Your problems remain.','Welcome back. Unfortunately, your score is still here.','Right. Where were we? Ah yes. Struggling.','And we are back. I hope the advertisement gave you time to reflect.'];
...
// Ad banter bypasses Petty's normal "already speaking" guard. That guard is
// useful for gameplay chatter, but it was swallowing the post-ad punchline.
...
function playPlaceholderAd(onComplete=()=>{}){
  if(adOpen)return;adOpen=true;beforeAd();
  setTimeout(()=>{
    ...
    const t=setInterval(()=>{n--;const el=overlay.querySelector('#adCount');if(el)el.textContent=n;if(n<=0){clearInterval(t);overlay.remove();setTimeout(()=>{afterAd();adOpen=false;onComplete()},POST_AD_PAUSE)}},1000);
  },PRE_AD_PAUSE);
}
