...
let adOpen=false,pending=false,resumeRetry=false,boundaryLocked=false,lastAdAttempt=+(localStorage.getItem(AK('lastAttempt'))||0);
...
function playPlaceholderAd(onComplete=()=>{}){if(adOpen||NO_ADS){boundaryLocked=false;return}adOpen=true;window.__PETTY_AD_BANTER_LOCK=true;try{window.speechSynthesis?.cancel?.()}catch{}beforeAd();setTimeout(()=>{const overlay=document.createElement('div');overlay.style.cssText='position:fixed;inset:0;background:#000;z-index:90;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#fff;font-family:Chakra Petch,system-ui,sans-serif;text-align:center;padding:24px';overlay.innerHTML='<div style="font-size:12px;letter-spacing:2px;color:#9c9ca8;margin-bottom:10px">AD NETWORK NOT CONNECTED YET</div><div id="adTitle" style="font-family:Teko,Arial Black,sans-serif;font-size:26px;margin-bottom:18px">SIMULATED AD — <span id="adCount">'+SIM_AD_SECONDS+'</span>s</div><div id="adSub" style="font-size:11px;color:#7a7a86;max-width:280px">Placeholder for the real mobile ad.</div>';document.body.appendChild(overlay);let n=SIM_AD_SECONDS;const t=setInterval(()=>{n--;const el=overlay.querySelector('#adCount');if(el)el.textContent=n;if(n<=0){clearInterval(t);const title=overlay.querySelector('#adTitle'),sub=overlay.querySelector('#adSub');if(title)title.innerHTML='AD COMPLETE';if(sub)sub.innerHTML='<span style="display:inline-block;margin-top:8px;font-size:14px;color:#fff;font-weight:800;letter-spacing:1px">TAP TO CONTINUE</span>';overlay.style.cursor='pointer';const finish=e=>{e.preventDefault();overlay.removeEventListener('pointerup',finish);primeGameAudio();if(title)title.innerHTML='PETTY HAS ONE MORE THING TO SAY…';if(sub)sub.textContent='';afterAd(()=>{primeGameAudio();overlay.remove();adOpen=false;window.__PETTY_AD_BANTER_LOCK=false;boundaryLocked=false;onComplete()})};overlay.addEventListener('pointerup',finish,{once:true})}},1000)},PRE_AD_PAUSE)}
...
function updateDue(){if(NO_ADS){pending=false;return}if(hasPass()&&!AD_TEST){pending=false;return}const now=currentAttempt(),gap=now-lastAdAttempt,hot=streak()>=2;if(AD_TEST){pending=gap>=1;return}pending=(gap>=5&&!hot)||gap>=9}
...
document.addEventListener('click',e=>{
  if(NO_ADS)return;
  const isRetry=e.target?.matches?.('#retry'),isClose=e.target?.matches?.('#close');
  if(!isRetry&&!isClose)return;
  if(resumeRetry){resumeRetry=false;return}
  if(boundaryLocked){e.preventDefault();e.stopImmediatePropagation();return}
  updateDue();
  if(!pending||(hasPass()&&!AD_TEST)||adOpen)return;
  const now=currentAttempt(),gap=now-lastAdAttempt;
  if(!AD_TEST&&streak()>=2&&gap<9)return;
  // Lock synchronously in the same capture handler so rapid re-taps cannot queue
  // duplicate retries before the ad overlay has actually mounted.
  boundaryLocked=true;
  pending=false;
  lastAdAttempt=now;
  ...
  if(isRetry){
    ...
    setTimeout(()=>playPlaceholderAd(()=>{resumeRetry=true;document.querySelector('#retry')?.click()}),0);
  }else{
    e.preventDefault();
    e.stopImmediatePropagation();
    setTimeout(()=>playPlaceholderAd(),0);
  }
},true);
})();
