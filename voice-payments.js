/* 99% IMPOSSIBLE — Stripe purchases + signed entitlements. */
(()=>{
'use strict';
...
async function purchase(item){if(!premium.includes(item))return false;if(owned.has(item))return true;try{const data=await post('/api/create-checkout',{item,buyerKey:buyerKey()});if(!data.url)throw new Error('missing_checkout_url');location.assign(data.url);return true}catch(err){console.error('[N99 PAYMENTS] checkout failed',err?.message||err);alert('Checkout is unavailable right now. Please try again.');return false;}}
window.N99Entitlements={ready,isOwned:item=>item==='petty'||owned.has(item),purchase,getOwned:()=>[...owned]};
(async()=>{buyerKey();await Promise.all(premium.map(verifySaved));await processReturn();emit();readyResolve([...owned]);})().catch(err=>{console.error('[N99 PAYMENTS] entitlement init failed',err?.message||err);readyResolve([...owned])});
})();
